...
from opencmiss.utils.zinc.field import findOrCreateFieldCoordinates, get_group_list, findOrCreateFieldFiniteElement, \
    findOrCreateFieldStoredString, findOrCreateFieldGroup, findOrCreateFieldNodeGroup, \
...
class MeshType_3d_dataDownsample1(Scaffold_base):
    ...
    def generateBaseMesh(cls, region, options):
        ...
        markerName = findOrCreateFieldStoredString(fieldModule, 'marker_name')
        ...
        sir.createStreamresourceFile('C:\\Users\\mlin865\\SPARC Datasets\\Powley\\Efferent\\ex\\dorsal\\11138_Efferents_Dorsal_Fiber-1_QF0.ex')

        tmpRegion.read(sir)

        nodeset = fieldModule.findNodesetByFieldDomainType(Field.DOMAIN_TYPE_NODES)
        datapointset = fieldModule.findNodesetByFieldDomainType(Field.DOMAIN_TYPE_DATAPOINTS)
        print('Check datapoints', datapointset.getSize())
        ...
        nodeIter = datapointset.createNodeiterator()
        datapoint = nodeIter.next()
        ...
        while datapoint.isValid():
            ...
            fieldcache.setNode(datapoint)
            result, x = coordinates.getNodeParameters(fieldcache, -1, Node.VALUE_LABEL_VALUE, 1, 3)
            mName = markerName.evaluateString(fieldcache)
            resultRadius, r = radius.getNodeParameters(fieldcache, -1, Node.VALUE_LABEL_VALUE, 1, 1)
            resultRGB, colour = rgb.getNodeParameters(fieldcache, -1, Node.VALUE_LABEL_VALUE, 1, 3)

            # do stuff with marker
            datapoint = nodeIter.next()
            markerIdList.append(identifier)
            markerXList.append(x)
            markerNameList.append(mName)
            markerRadiusList.append(r)
            markerRGBList.append(colour)
        ...
        for group in allGroups:
            ...
            meshGroup = group.getFieldElementGroup(mesh1d).getMeshGroup()
            ...
            if clearedChildList[parentNode][0] == 0: # No child / child already removed
                ...
                if childAtEndOfBranch:
                    ...
                    while not childAtEndOfBranch:
                        count += 1
                        assert count < 50000, 'Trapped in while loop'
        ...
        # markerLocation = findOrCreateFieldStoredMeshLocation(fm, mesh1d, name="marker_location")
        ...
        #
        # markersList = []
        # for i in range(1, len(markerXList)):
        #     markersList.append({"group": markerNameList[i], "x": markerXList[i] })
        #
        # nodeIdentifier = 1
        ...
        #     print(marker["x"])
        #     markerLocation.assignReal(fm, marker["x"])
