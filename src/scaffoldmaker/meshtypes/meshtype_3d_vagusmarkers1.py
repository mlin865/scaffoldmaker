"""
Generates a 1D network path based on markers on vagal trunk from Japanese dataset
"""
...
class MeshType_3d_vagusmarkers1(Scaffold_base):
    '''
    Generates a 1-D vagus path mesh from Japanese dataset.
    '''

    @staticmethod
    def getName():
        return '3D Vagus markers 1'
    ...
    def generateBaseMesh(cls, region, options):
        ...
        sir.createStreamresourceFile('C:\\Users\\mlin865\\Vagus\\Japanese dataset_exf\\leftVagusSegmentation.exdata')
        ...
        coordinates = fieldModule.findFieldByName('marker_data_coordinates').castFiniteElement()
        ...
        for n in range(len(xList) - 1):
            d1= [xList[n + 1][c] - xList[n][c] for c in range(3)]
            d1List.append(d1)
        d1List.append(d1)
        ...
        for n1 in range(len(xList)):
            ...
            cache.setNode(node)
            coordinates.setNodeParameters(cache, -1, Node.VALUE_LABEL_VALUE, 1, xList[n1])
            coordinates.setNodeParameters(cache, -1, Node.VALUE_LABEL_D_DS1, 1, d1List[n1])
